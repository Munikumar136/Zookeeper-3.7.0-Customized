...
/**
 * Validate that open/close session request of a local session to not propagate to other machines in
 * the quorum. We verify this by checking that these request doesn't show up in committedLog on
 * other machines.
 */
  ...
  private void validateRequestLog(long sessionId, int peerId) {
    ...
    for (Proposal p : db.getCommittedLog()) {
      assertFalse(p.request.sessionId == sessionId,
          "Should not see " + Request.op2String(p.request.type) + " request from local session 0x"
              + session + " on the " + peerType);
    }
  }

  /**
   * Test that a CloseSession request generated by both the server (client disconnect) or by the
   * client (client explicitly issue close()) doesn't get committed by the ensemble
   */
