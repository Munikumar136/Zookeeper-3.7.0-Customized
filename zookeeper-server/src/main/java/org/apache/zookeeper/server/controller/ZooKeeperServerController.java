...
/**
 * Class which accepts commands to modify ZooKeeperServer state or Connection state at runtime for
 * the purpose of single machine integration testing. Not meant to be used in production. It is
 * recommended to use this in conjunction with the CommandListener HttpServer and CommandClient.
 *
 */
...
public class ZooKeeperServerController {
  ...
  public synchronized boolean isReady() {
    return this.cnxnFactory != null && this.quorumPeer != null && this.quorumPeer.isRunning()
  ...
  public void processCommand(ControlCommand command) {
    ...
    switch (command.getAction()) {
      ...
      case REJECTCONNECTIONS:
        ...
        break;
      case ADDDELAY:
        cnxnFactory.delayResponses(command.getParameter() == null ? DEFAULT_DELAY_MS
            : Long.decode(command.getParameter()));
        break;
      case NORESPONSE:
        if (command.getParameter() == null) {
