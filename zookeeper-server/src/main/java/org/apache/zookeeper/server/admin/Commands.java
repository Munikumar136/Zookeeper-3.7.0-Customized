...
/**
 * Class containing static methods for registering and running Commands, as well as default Command
 * definitions.
 *
 * @see Command
...
public class Commands {
  ...
  static final Logger LOG = LogManager.getLogger(Commands.class);
  ...
  /**
   * Registers the given command. Registered commands can be run by passing any of their names to
   * runCommand.
   */
  ...
  /**
   * Run the registered command with name cmdName. Commands should not produce any exceptions; any
   * (anticipated) errors should be reported in the "error" entry of the returned map. Likewise, if
   * no command with the given name is registered, this will be noted in the "error" entry.
   *
   * @param cmdName
   * @param zkServer
   * @param kwargs String-valued keyword arguments to the command (may be null if command requires
   *        no additional arguments)
   * @return Map representing response to command containing at minimum: - "command" key containing
   *         the command's primary name - "error" key containing a String error message or null if
   *         no error
   */
  public static CommandResponse runCommand(String cmdName, ZooKeeperServer zkServer,
  ...
  /**
   * Returns the commands registered under cmdName with registerCommand, or null if no command is
  ...
  /**
   * Information on client connections to server. Returned Map contains: - "connections": list of
   ...
   * "expiry_time_to_session_ids": Map&lt;Long, Set&lt;Long&gt;&gt; time -&gt; sessions IDs of
   * sessions that expire at time - "session_id_to_ephemeral_paths": Map&lt;Long,
   * Set&lt;String&gt;&gt; session ID -&gt; ephemeral paths created by that session
   * 
  ...
  /**
   * The current trace mask. Returned map contains: - "tracemask": Long
   */
  ...
  /**
   * Is this server in read-only mode. Returned map contains: - "is_read_only": Boolean
   */
  ...
  /**
   * Command returns information of the last snapshot that zookeeper server has finished saving to
   * disk. During the time between the server starts up and it finishes saving its first snapshot,
   * the command returns the zxid and last modified time of the snapshot file used for restoration
   * at server startup. Returned map contains: - "zxid": String - "timestamp": Long
   */
  ...
  /**
   * Some useful info for monitoring. Returned map contains: - "version": String server version -
   * "avg_latency": Long - "max_latency": Long - "min_latency": Long - "packets_received": Long -
   * "packets_sents": Long - "num_alive_connections": Integer - "outstanding_requests": Long number
   * of unprocessed requests - "server_state": "leader", "follower", or "standalone" -
   * "znode_count": Integer - "watch_count": Integer - "ephemerals_count": Integer -
   * "approximate_data_size": Long - "open_file_descriptor_count": Long (unix only) -
   * "max_file_descriptor_count": Long (unix only) - "fsync_threshold_exceed_count": Long -
   * "non_mtls_conn_count": Long - "non_mtls_remote_conn_count": Long - "non_mtls_local_conn_count":
   * Long - "followers": Integer (leader only) - "synced_followers": Integer (leader only) -
  ...
  /**
   * Sets the trace mask. Required arguments: - "traceMask": Long Returned Map contains: -
   * "tracemask": Long
   */
  public static class SetTraceMaskCommand extends CommandBase {
    ...
    public CommandResponse run(ZooKeeperServer zkServer, Map<String, String> kwargs) {
      ...
      try {
        ...
        response.put("error",
            "setTraceMask requires long traceMask argument, got " + kwargs.get("traceMask"));
        return response;
      }
  ...
  /**
   * Server information. Returned map contains: - "version": String version of server - "read_only":
   * Boolean is server in read-only mode - "server_stats": ServerStats object - "node_count":
   * Integer
   */
  ...
  /**
   * Information on observer connections to server. Returned Map contains: - "synced_observers":
   * Integer (leader/follower only) - "observers": list of observer learner handler info objects
  ...
  /**
   * Returns the current ensemble configuration information. It provides list of current voting
  ...
  public static class VotingViewCommand extends CommandBase {
    ...
    private static class QuorumServerView {
      ...
      public QuorumServerView(QuorumPeer.QuorumServer quorumServer) {
        ...
        this.learnerType =
            quorumServer.type.equals(LearnerType.PARTICIPANT) ? "participant" : "observer";
      ...
      private static List<String> getMultiAddressString(MultipleAddresses multipleAddresses) {
        ...
        return multipleAddresses.getAllAddresses().stream().map(QuorumServerView::getAddressString)
      ...
      private static String getAddressString(InetSocketAddress address) {
        ...
        return String.format("%s:%d", QuorumPeer.QuorumServer.delimitedHostString(address),
            address.getPort());
      }
    }
   ...
   * "session_id_to_watched_paths": Map&lt;Long, Set&lt;String&gt;&gt; session ID -&gt; watched
   * paths
   * 
  ...
  /**
   * Watch information aggregated by path. Returned Map contains: - "path_to_session_ids":
   * Map&lt;String, Set&lt;Long&gt;&gt; path -&gt; session IDs of sessions watching path
   * 
  ...
  /**
   * Returns the current phase of Zab protocol that peer is running. It can be in one of these
   * phases: ELECTION, DISCOVERY, SYNCHRONIZATION, BROADCAST
   */
  public static class ZabStateCommand extends CommandBase {
    ...
    public CommandResponse run(ZooKeeperServer zkServer, Map<String, String> kwargs) {
      ...
      if (zkServer instanceof QuorumZooKeeperServer) {
        ...
        boolean voting = (voter != null && voter.addr.equals(peer.getQuorumAddress())
