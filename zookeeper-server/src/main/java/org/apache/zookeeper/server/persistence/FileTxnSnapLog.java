...
public class FileTxnSnapLog {
  ...
  private static final String EMPTY_SNAPSHOT_WARNING =
      "No snapshot found, but there are log entries. ";

  /**
   * This listener helps the external apis calling restore to gather information while the data is
   * being restored.
   */
  ...
  /**
   * Finalizing restore of data tree through a set of operations (replaying transaction logs,
  ...
  public FileTxnSnapLog(File dataDir, File snapDir) throws IOException {
    ...
    autoCreateDB = Boolean
        .parseBoolean(System.getProperty(ZOOKEEPER_DB_AUTOCREATE, ZOOKEEPER_DB_AUTOCREATE_DEFAULT));
  }
  ...
  /**
   * whether to force the write of an initial snapshot after a leader election, to address
   * ZOOKEEPER-3781 after upgrading from Zookeeper 3.4.x.
   * 
   * @return true if an initial snapshot should be written even if not otherwise required, false
   *         otherwise.
   */
  ...
  /**
   * this function restores the server database after reading from the snapshots and transaction
   * logs
   * 
  ...
  public long restore(DataTree dt, Map<Long, Integer> sessions, PlayBackListener listener)
    ...
    ServerMetrics.getMetrics().STARTUP_SNAP_LOAD_TIME
        .add(Time.currentElapsedTime() - snapLoadingStartTime);
    ...
    if (-1L == deserializeResult) {
      /*
       * this means that we couldn't find any snapshot, so we need to initialize an empty database
       * (reported in ZOOKEEPER-2325)
       */
  ...
  /**
   * This function will fast forward the server database to have the latest transactions in it. This
   * is the same as restore, but only reads from the transaction logs and not restores from a
   * snapshot.
   * 
    ...
    try {
      while (true) {
        ...
        if (hdr.getZxid() < highestZxid && highestZxid != 0) {
          LOG.error("{}(highestZxid) > {}(next log) for type {}", highestZxid, hdr.getZxid(),
              hdr.getType());
        } else {
        ...
        try {
          ...
          throw new IOException(
              "Failed to process transaction type: " + hdr.getType() + " error: " + e.getMessage(),
              e);
        }
   ...
   *
   * @param zxid starting zxid
   * @param fastForward true if the iterator should be fast forwarded to point to the txn of a given
   *        zxid, else the iterator will point to the starting txn of a txnlog that may contain txn
   *        of a given zxid
   * @return TxnIterator
   * @throws IOException
   */
  ...
  public void processTransaction(TxnHeader hdr, DataTree dt, Map<Long, Integer> sessions,
    ...
    /*
     * Snapshots are lazily created. So when a snapshot is in progress, there is a chance for later
     * transactions to make into the snapshot. Then when the snapshot is restored, NONODE/NODEEXISTS
     * errors could occur. It should be safe to ignore these.
     */
    if (rc.err != Code.OK.intValue()) {
      LOG.debug("Ignoring processTxn failure hdr: {}, error: {}, path: {}", hdr.getType(), rc.err,
          rc.path);
    }
  }
   ...
   * @param sessionsWithTimeouts the session timeouts to be serialized onto disk
  ...
  public void save(DataTree dataTree, ConcurrentHashMap<Long, Integer> sessionsWithTimeouts,
    ...
    try {
      ...
      if (snapshotFile.length() == 0) {
        /*
         * This may be caused by a full disk. In such a case, the server will get stuck in a loop
         * where it tries to write a snapshot out to disk, and ends up creating an empty file
         * instead. Doing so will eventually result in valid snapshots being removed during cleanup.
         */
        ...
        /*
         * Something else went wrong when writing the snapshot out to disk. If this snapshot file is
         * invalid, when restarting, ZooKeeper will skip it, and find the last known good snapshot
         * instead.
         */
      }
      throw e;
   ...
   * @return the list of n recent valid snapshots, with the most recent in front
   * @throws IOException
   */
  ...
  /**
   * get the snapshot logs which may contain transactions newer than the given zxid. This includes
   * logs with starting zxid greater than given zxid, as well as the newest transaction log with
   * starting zxid less than given zxid. The latter log file may contain transactions beyond given
   * zxid.
   * 
   * @param zxid the zxid that contains logs greater than zxid
