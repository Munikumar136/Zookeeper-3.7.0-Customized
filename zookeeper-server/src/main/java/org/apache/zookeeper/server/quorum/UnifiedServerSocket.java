...
/**
 * A ServerSocket that can act either as a regular ServerSocket, as a SSLServerSocket, or as both,
 * depending on the constructor parameters and on the type of client (TLS or plaintext) that
 * connects to it. The constructors have the same signature as constructors of ServerSocket, with
 * the addition of two parameters at the beginning:
 * <ul>
 * <li>X509Util - provides the SSL context to construct a secure socket when a client connects with
 * TLS.</li>
 * <li>boolean allowInsecureConnection - when true, acts as a hybrid server socket (plaintext /
 * TLS). When false, acts as a SSLServerSocket (rejects plaintext connections).</li>
 * </ul>
 * The <code>!allowInsecureConnection</code> mode is needed so we can update the SSLContext (in
 * particular, the key store and/or trust store) without having to re-create the server socket. By
 * starting with a plaintext socket and delaying the upgrade to TLS until after a client has
 * connected and begins a handshake, we can keep the same UnifiedServerSocket instance around, and
 * replace the default SSLContext in the provided X509Util when the key store and/or trust store
...
public class UnifiedServerSocket extends ServerSocket {
  ...
  /**
   * Creates an unbound unified server socket by calling {@link ServerSocket#ServerSocket()}. Secure
   * client connections will be upgraded to TLS once this socket detects the ClientHello message
   * (start of a TLS handshake). Plaintext client connections will either be accepted or rejected
   * depending on the value of the <code>allowInsecureConnection</code> parameter.
   * 
   ...
   * {@link ServerSocket#ServerSocket(int)}. Secure client connections will be upgraded to TLS once
   * this socket detects the ClientHello message (start of a TLS handshake). Plaintext client
   * connections will either be accepted or rejected depending on the value of the
   * <code>allowInsecureConnection</code> parameter.
   * 
   * @param x509Util the X509Util that provides the SSLContext to use for secure connections.
   * @param allowInsecureConnection if true, accept plaintext connections, otherwise close them.
  ...
  /**
   * Creates a unified server socket bound to the specified port, with the specified backlog, by
   * calling {@link ServerSocket#ServerSocket(int, int)}. Secure client connections will be upgraded
   * to TLS once this socket detects the ClientHello message (start of a TLS handshake). Plaintext
   * client connections will either be accepted or rejected depending on the value of the
   * <code>allowInsecureConnection</code> parameter.
   * 
   * @param x509Util the X509Util that provides the SSLContext to use for secure connections.
   * @param allowInsecureConnection if true, accept plaintext connections, otherwise close them.
  ...
  /**
   * Creates a unified server socket bound to the specified port, with the specified backlog, and
   * local IP address to bind to, by calling
   * {@link ServerSocket#ServerSocket(int, int, InetAddress)}. Secure client connections will be
   * upgraded to TLS once this socket detects the ClientHello message (start of a TLS handshake).
   * Plaintext client connections will either be accepted or rejected depending on the value of the
   * <code>allowInsecureConnection</code> parameter.
   * 
   * @param x509Util the X509Util that provides the SSLContext to use for secure connections.
   * @param allowInsecureConnection if true, accept plaintext connections, otherwise close them.
  ...
  /**
   * The result of calling accept() on a UnifiedServerSocket. This is a Socket that doesn't know if
   * it's using plaintext or SSL/TLS at the time when it is created. Calling a method that indicates
   * a desire to read or write from the socket will cause the socket to detect if the connected
   * client is attempting to establish a TLS or plaintext connection. This is done by doing a
   * blocking read of 5 bytes off the socket and checking if the bytes look like the start of a TLS
   * ClientHello message. If it looks like the client is attempting to connect with TLS, the
   * internal socket is upgraded to a SSLSocket. If not, any bytes read from the socket are pushed
   * back to the input stream, and the socket continues to be treated as a plaintext socket.
   *
   ...
   * </ul>
   *
   * Calling other socket methods (i.e option setters such as {@link Socket#setTcpNoDelay(boolean)})
   * does not trigger mode detection.
   *
   * Because detecting the mode is a potentially blocking operation, it should not be done in the
   * accepting thread. Attempting to read from or write to the socket in the accepting thread opens
   * the caller up to a denial-of-service attack, in which a client connects and then does nothing.
   * This would prevent any other clients from connecting. Passing the socket returned by accept()
   * to a separate thread which handles all read and write operations protects against this DoS
   * attack.
   *
   * Callers can check if the socket has been upgraded to TLS by calling
   * {@link UnifiedSocket#isSecureSocket()}, and can get the underlying SSLSocket by calling
   * {@link UnifiedSocket#getSslSocket()}.
   */
  public static class UnifiedSocket extends Socket {

    private enum Mode {
      UNKNOWN, PLAINTEXT, TLS
    }
     ...
     */
    private UnifiedSocket(X509Util x509Util, boolean allowInsecureConnection,
    ...
    /**
     * Detects the socket mode, see comments at the top of the class for more details. This
     * operation will block for up to {@link X509Util#getSslHandshakeTimeoutMillis()} milliseconds
     * and should not be called in the accept() thread if possible.
     * 
     * @throws IOException
     */
    private void detectMode() throws IOException {
      ...
      try {
        ...
        // Didn't read anything within the timeout, fallthrough and assume the connection is
        // plaintext.
    ...
    /**
     * Returns the underlying socket, detecting the socket mode if it is not yet known. This is a
     * potentially blocking operation and should not be called in the accept() thread.
     * 
    ...
    /**
     * Returns the underlying SSLSocket if the mode is TLS. If the mode is UNKNOWN, causes mode
     * detection which is a potentially blocking operation. If the mode ends up being PLAINTEXT,
     * this will throw a SocketException, so callers are advised to only call this method after
     * checking that {@link UnifiedSocket#isSecureSocket()} returned true.
     * 
    ...
    /**
     * See {@link Socket#connect(SocketAddress)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#connect(SocketAddress, int)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#getRemoteSocketAddress()}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#getLocalSocketAddress()}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#getInputStream()}. If the socket mode has not yet been detected, the first
     * read from the returned input stream will trigger mode detection, which is a potentially
     * blocking operation. This means the accept() thread should avoid reading from this input
     * stream if possible.
     */
    @Override
    ...
    /**
     * See {@link Socket#getOutputStream()}. If the socket mode has not yet been detected, the first
     * read from the returned input stream will trigger mode detection, which is a potentially
     * blocking operation. This means the accept() thread should avoid reading from this input
     * stream if possible.
     */
    ...
    /**
     * See {@link Socket#setTcpNoDelay(boolean)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#setSoLinger(boolean, int)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#sendUrgentData(int)}. Calling this method triggers mode detection, which is
     * a potentially blocking operation, so it should not be done in the accept() thread.
     */
    @Override
    ...
    /**
     * See {@link Socket#setOOBInline(boolean)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#setSendBufferSize(int)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#setReceiveBufferSize(int)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#getReceiveBufferSize()}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#setKeepAlive(boolean)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#setReuseAddress(boolean)}. Calling this method does not trigger mode
     * detection.
     */
    @Override
    ...
    /**
     * See {@link Socket#setPerformancePreferences(int, int, int)}. Calling this method does not
  ...
  /**
   * An input stream for a UnifiedSocket. The first read from this stream will trigger mode
   * detection on the underlying UnifiedSocket.
   */
  private static class UnifiedInputStream extends InputStream {
    ...
    /**
     * Note: SocketInputStream has optimized implementations of bulk-read operations, so we need to
     * call them directly instead of relying on the base-class implementation which just calls the
     * single-byte read() over and over. Not implementing these results in awful performance.
     */
    @Override
