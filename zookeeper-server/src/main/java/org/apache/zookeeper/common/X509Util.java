 ...
 *
 * Performance testing done by Facebook engineers shows that on Intel x86_64 machines, Java9
 * performs better with GCM and Java8 performs better with CBC, so these seem like reasonable
 * defaults.
 */
public abstract class X509Util implements Closeable, AutoCloseable {
  ...
  public enum ClientAuth {
    NONE(io.netty.handler.ssl.ClientAuth.NONE), WANT(
        io.netty.handler.ssl.ClientAuth.OPTIONAL), NEED(io.netty.handler.ssl.ClientAuth.REQUIRE);
  ...
  /**
   * Returns the config property key that controls the amount of time, in milliseconds, that the
   * first UnifiedServerSocket read operation will block for when trying to detect the client mode
   * (TLS or PLAINTEXT).
   *
  ...
  private SSLContextAndOptions createSSLContextAndOptions() throws SSLContextException {
    /*
     * Since Configuration initializes the key store and trust store related configuration from
     * system property. Reading property from configuration will be same reading from system
     * property
     */
  ...
  /**
   * Returns the max amount of time, in milliseconds, that the first UnifiedServerSocket read()
   * operation should block for when trying to detect the client mode (TLS or PLAINTEXT). Defaults
   * to {@link X509Util#DEFAULT_HANDSHAKE_DETECTION_TIMEOUT_MILLIS}.
   *
  ...
  public int getSslHandshakeTimeoutMillis() {
    try {
      ...
      LOG.error("Error parsing config property {}", getSslHandshakeDetectionTimeoutMillisProperty(),
          e);
      return DEFAULT_HANDSHAKE_DETECTION_TIMEOUT_MILLIS;
    }
  }
    ...
    if (supplierContextClassName != null) {
      ...
      try {
        ...
        throw new SSLContextException(
            "Could not retrieve the SSLContext from supplier source '" + supplierContextClassName
  ...
  public SSLContextAndOptions createSSLContextAndOptionsFromConfig(ZKConfig config)
      throws SSLContextException {
    ...
    // But if a user wanna specify one, location is required. Password defaults to empty string if
    // it is not
    ...
    if (keyStoreLocationProp.isEmpty()) {
      ...
      try {
        ...
        throw new SSLContextException(
            "Bad value for " + sslKeystoreTypeProperty + ": " + keyStoreTypeProp, e);
    ...
    boolean sslServerHostnameVerificationEnabled =
        config.getBoolean(this.getSslHostnameVerificationEnabledProperty(), false);
    ...
    if (trustStoreLocationProp.isEmpty()) {
      ...
      try {
        trustManagers = new TrustManager[] {createTrustManager(trustStoreLocationProp,
  ...
  public static KeyStore loadTrustStore(String trustStoreLocation, String trustStorePassword,
   ...
   * @param keyStoreTypeProp must be JKS, PEM, PKCS12, BCFKS or null. If null, attempts to
   *        autodetect the key store type from the file extension (e.g. .jks / .pem).
   ...
   * @param trustStoreTypeProp must be JKS, PEM, PKCS12, BCFKS or null. If null, attempts to
   *        autodetect the trust store type from the file extension (e.g. .jks / .pem).
   * @param crlEnabled enable CRL (certificate revocation list) checks.
   * @param ocspEnabled enable OCSP (online certificate status protocol) checks.
   * @param serverHostnameVerificationEnabled if true, verify hostnames of remote servers that
   *        client sockets created by this X509Util connect to.
   * @param clientHostnameVerificationEnabled if true, verify hostnames of remote clients that
   *        server sockets created by this X509Util accept connections from.
  ...
  public static X509TrustManager createTrustManager(String trustStoreLocation,
      String trustStorePassword, String trustStoreTypeProp, boolean crlEnabled, boolean ocspEnabled,
