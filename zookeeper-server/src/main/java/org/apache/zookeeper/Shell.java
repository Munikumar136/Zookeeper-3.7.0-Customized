...
/*
 * This file copied from Hadoop's security branch, with the following changes: 1. package changed
 * from org.apache.hadoop.util to org.apache.zookeeper. 2. Usage of Hadoop's Configuration class
 * removed since it is not available in Zookeeper: instead, system properties are used. 3. The
 * deprecated getUlimitMemoryCommand() method removed since it is not needed.
 */
 ...
 *
 * <code>Shell</code> can be used to run unix commands like <code>du</code> or <code>df</code>. It
 * also offers facilities to gate commands by time-intervals.
 */
public abstract class Shell {
   ...
   * This is only relevant when we are forking a process from within the Mapper or the Reducer
   * implementations. Also see Hadoop Pipes and Hadoop Streaming.
   *
   * It also checks to ensure that we are running on a *nix platform else (e.g. in Cygwin/Windows)
   * it returns <code>null</code>.
   * 
   * @param memoryLimit virtual memory limit
   * @return a <code>String[]</code> with the ulimit command arguments or <code>null</code> if we
   *         are running on a non *nix platform or if the limit is unspecified.
   */
  ...
  public static class ShellCommandExecutor extends Shell {
     ...
     * @param dir If not-null, specifies the directory which should be set as the current working
     *        directory for the command. If null, the current working directory is not modified.
     * @param env If not-null, environment of the command will include the key-value pairs specified
     *        in the map. If null, the current environment is not modified.
     * @param timeout Specifies the time in milliseconds, after which the command will be killed and
     *        the status marked as timedout. If 0, the command will not be timed out.
     */
    public ShellCommandExecutor(String[] execString, File dir, Map<String, String> env,
        long timeout) {
